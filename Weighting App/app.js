(function () {
  ...
  function activeVariableMap() {
    ...
    VARIABLE_ORDER.forEach((key) => {
      ...
      if (v.enabled && v.column && Object.keys(v.targets).length > 0) {
        out[key] = { getCategory: getterFor(key), targets: v.targets };
      }
    });
    return out;
  }
  ...
  function renderVarBody(body, key) {
    ...
    if (Object.keys(v.targets).length > 0) {
      body.appendChild(buildSliders(key));
    }
  }
  ...
  function buildSliders(key) {
    ...
    headRow.className = "slider-head-row";
    ["Category", "Sample", "Target", "Value"].forEach((h) => {
    ...
    def.categories.forEach((cat) => {
      ...
      slider.max = "1";
      slider.step = "0.01";
      slider.value = round(v.targets[cat] || 0, 2);

      const valEl = document.createElement("span");
      valEl.className = "target-val";
      valEl.textContent = round(v.targets[cat] || 0, 2).toFixed(2);

      slider.addEventListener("input", () => {
        const val = Math.max(0, Math.min(1, parseFloat(slider.value)));
        const others = def.categories.filter((c) => c !== cat);
        const otherSum = others.reduce((a, c) => a + (v.targets[c] || 0), 0);
        const remaining = 1 - val;
        const newTargets = {};
        newTargets[cat] = val;
        others.forEach((c) => {
          newTargets[c] = otherSum > 0 ? ((v.targets[c] || 0) / otherSum) * remaining : remaining / others.length;
        });
        v.targets = newTargets;
        valEl.textContent = val.toFixed(2);
        // update sibling sliders/labels in place without a full re-render
        Array.from(wrap.querySelectorAll(".slider-row")).forEach((r, i) => {
          const c = def.categories[i];
          if (c === cat) return;
          const s = r.querySelector('input[type="range"]');
          const t = r.querySelector(".target-val");
          s.value = round(newTargets[c], 2);
          t.textContent = round(newTargets[c], 2).toFixed(2);
        });
