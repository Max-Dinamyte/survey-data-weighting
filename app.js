(function () {
  ...
  const VARIABLES = {
    ...
    age: {
      ...
      numeric: true,
      defaultCutoffs: [18, 25, 35, 45, 55, 65],
    ...
    education: {
      ...
      synonyms: {
        ...
        "High school diploma": ["highschool", "hsdiploma", "highschoolgraduate", "ged", "highschoolgrad", "hsgrad"],
  ...
  // Not a demographic weighting variable, a sampling design variable,
  // rendered in its own "Sample design" section rather than the main
  // variable list. Has no fixed category scheme (unlike the 7 above) since
  // strata are entirely survey-specific; categories are adopted directly
  // from whatever distinct values are in the chosen column instead.
  const STRATIFICATION_KEY = "stratification";
  VARIABLES[STRATIFICATION_KEY] = {
    label: "Stratification variable",
    categories: [],
    keywords: ["stratum", "strata", "stratumid", "stratum_id", "psu", "segment", "cluster", "samplingstratum", "region", "state", "division", "geo", "geography"],
  };
  ...
  function suggestCategory(rawValue, categories, synonyms) {
    ...
    for (let i = 0; i < cats.length; i++) {
      ...
      for (let j = 0; j < syns.length; j++) {
        if (syns[j].length < 4) continue; // short synonyms match exactly only (above), never via substring
  ...
  // cutoffs[i] is the value at which bracket i begins; there's one cutoff
  // per bracket (including the floor for bracket 0), so removing a cutoff
  // cleanly merges into the bracket before it and adding one always opens a
  // new top bracket. Values below cutoffs[0] are excluded, same treatment
  // as any other unmapped value elsewhere in the app.
  function binNumeric(num, cutoffs, labels) {
    if (cutoffs.length === 0 || num < cutoffs[0]) return null;
    let idx = 0;
    for (let i = 1; i < cutoffs.length; i++) {
      if (num >= cutoffs[i]) idx = i;
      else break;
    }
    return labels[idx];
  }

  // Bracket labels are always computed from the cutoffs, never typed
  // independently: that disconnect (a free-text label next to an unrelated
  // number) was the actual source of confusion in the numeric bracket
  // editor. Only the boundary numbers are user input; the label is a live,
  // read-only readout of what those numbers produce.
  function computeNumericLabels(cutoffs) {
    return cutoffs.map((start, i) => {
      if (i === cutoffs.length - 1) return String(start) + "+";
      return String(start) + "\u2013" + String(cutoffs[i + 1] - 1);
    });
  }
  ...
  function rake(rows, activeVars, opts) {
    const n = rows.length;
    // Starting weights default to uniform 1s, but a stratified design's base
    // weights can be supplied instead. IPF converges to whichever valid
    // solution is closest (in cross-entropy terms) to the starting weights,
    // so seeding from base weights keeps the result anchored to the actual
    // sampling design rather than treating strata as just another margin
    // measured from scratch.
    let w = opts && opts.initialWeights ? opts.initialWeights.slice() : new Array(n).fill(1);
    const keys = Object.keys(activeVars).filter((k) => activeVars[k].targets && Object.keys(activeVars[k].targets).length > 0);
    ...
    if (keys.length === 0 || n === 0) {
      // No IPF margins to satisfy, but starting weights (e.g. pure base
      // weights with no demographic raking on top) should still respect a
      // requested trim rather than passing through uncapped.
      if (trimOn && n > 0) {
        w = w.map((x) => {
        ...
        const m0 = w.reduce((a, b) => a + b, 0) / n;
        if (m0 > 0) w = w.map((x) => x / m0);
      }
  ...
  function freshVarState(key) {
    ...
    const cutoffs = def.numeric ? def.defaultCutoffs.slice() : null;
    return {
      column: "",
      mode: def.numeric ? "numeric" : "map",
      valueMap: {},
      cutoffs: cutoffs,
      categories: def.numeric ? computeNumericLabels(cutoffs) : def.categories.slice(),
      targets: {},
      enabled: false,
    };
  }
  ...
  function resetStratificationState() {
    state.vars[STRATIFICATION_KEY] = freshVarState(STRATIFICATION_KEY);
    state.vars[STRATIFICATION_KEY].enforceOngoing = true;
  }
  ...
  function autoSuggestValueMap(key) {
    ...
    const rawValues = Object.keys(counts).filter((r) => r !== "");
    if (v.categories.length === 0) {
      // No predefined scheme (the stratification variable). The raw values
      // themselves become the categories, identity-mapped, since there's no
      // universal synonym dictionary to suggest against for an arbitrary
      // survey-specific stratum code.
      v.categories = rawValues.slice();
      const identityMap = {};
      rawValues.forEach((raw) => {
        identityMap[raw] = raw;
      });
      v.valueMap = identityMap;
      return;
    }
    const map = {};
    rawValues.forEach((raw) => {
  ...
  // Recomputes labels from the current cutoffs and reconciles them against
  // the previous label at each position: a boundary edit that just shifts
  // where "25–34" starts carries its target value over under the new label
  // rather than resetting it, a removed bracket's target is dropped, and a
  // newly added bracket starts at target 0.
  function syncNumericCategories(key) {
    const v = state.vars[key];
    const oldCategories = v.categories;
    const newCategories = computeNumericLabels(v.cutoffs);
    const len = Math.min(oldCategories.length, newCategories.length);
    for (let i = 0; i < len; i++) {
      const oldName = oldCategories[i];
      const newName = newCategories[i];
    ...
    for (let i = newCategories.length; i < oldCategories.length; i++) {
      delete v.targets[oldCategories[i]];
      Object.keys(v.valueMap).forEach((raw) => {
        if (v.valueMap[raw] === oldCategories[i]) delete v.valueMap[raw];
      });
    }
    for (let i = oldCategories.length; i < newCategories.length; i++) {
      if (!(newCategories[i] in v.targets)) v.targets[newCategories[i]] = 0;
    }
    v.categories = newCategories;
  }
  ...
  function removeCategory(key, name) {
    ...
    if (def.numeric && v.cutoffs) {
      // One cutoff per bracket now, so removing this bracket's own cutoff
      // is enough. The bracket before it naturally extends to cover the
      // gap, since its span always runs up to whatever the next surviving
      // cutoff is.
      v.cutoffs.splice(idx, 1);
      syncNumericCategories(key);
      return true;
    }
  ...
  function addCategory(key) {
    ...
    const def = VARIABLES[key];
    if (def.numeric && v.cutoffs) {
      const last = v.cutoffs.length > 0 ? v.cutoffs[v.cutoffs.length - 1] : 18;
      v.cutoffs.push(last + 10);
      syncNumericCategories(key);
      return v.categories[v.categories.length - 1];
    }
    const label = uniqueCategoryName(v, "New category");
  ...
  // Design/base weight for a stratified sample: population share divided by
  // sample share, per stratum. A row whose stratum is unmapped gets no
  // correction (stays at 1) rather than being silently dropped, the same
  // convention as an unmapped value anywhere else in the app. Returns null
  // when stratification isn't in use, so callers can tell "no correction
  // needed" apart from "correction is exactly uniform."
  function computeBaseWeights() {
    const v = state.vars[STRATIFICATION_KEY];
    if (!v || !v.enabled || !v.column || Object.keys(v.targets).length === 0) return null;
    const getCat = getterFor(STRATIFICATION_KEY);
    const up = unweightedProps(state.data, getCat);
    const targets = normalizedTargets(v.targets);
    const n = state.data.length;
    const w = new Array(n).fill(1);
    for (let i = 0; i < n; i++) {
      const cat = getCat(state.data[i]);
      if (cat == null) continue;
      const tgt = targets[cat];
      const smp = up.props[cat];
      if (tgt === undefined || !smp) continue;
      w[i] = tgt / smp;
    }
    const m = w.reduce((a, b) => a + b, 0) / n;
    if (m > 0) {
      for (let i = 0; i < n; i++) w[i] = w[i] / m;
    }
    return w;
  }
  ...
  function activeVariableMap() {
    ...
    const strat = state.vars[STRATIFICATION_KEY];
    if (strat && strat.enabled && strat.enforceOngoing && strat.column && Object.keys(strat.targets).length > 0) {
      out[STRATIFICATION_KEY] = { getCategory: getterFor(STRATIFICATION_KEY), targets: normalizedTargets(strat.targets) };
    }
    return out;
  }
  ...
  const stratificationCardEl = document.getElementById("stratification-card");
  const stratEnforceRow = document.getElementById("strat-enforce-row");
  const stratEnforceCheckbox = document.getElementById("strat-enforce-ongoing");
  ...
  function handleFile(file) {
    ...
    Papa.parse(file, {
      ...
      complete: function (res) {
        ...
        const stratGuess = guessColumn(hdrs, VARIABLES[STRATIFICATION_KEY].keywords);
        if (stratGuess) {
          state.vars[STRATIFICATION_KEY].column = stratGuess;
          autoMapColumn(STRATIFICATION_KEY);
        }
  ...
  resetBtn.addEventListener("click", () => {
    ...
    resetAllVarState();
    resetStratificationState();
    stratificationCardEl.innerHTML = "";
    fileInput.value = "";
  ...
  function renderStratificationSection() {
    stratificationCardEl.innerHTML = "";
    stratificationCardEl.appendChild(buildVarCard(STRATIFICATION_KEY));
    const v = state.vars[STRATIFICATION_KEY];
    stratEnforceRow.hidden = !v.enabled;
    stratEnforceCheckbox.checked = v.enforceOngoing;
  }

  stratEnforceCheckbox.addEventListener("change", () => {
    state.vars[STRATIFICATION_KEY].enforceOngoing = stratEnforceCheckbox.checked;
    renderDiagnostics();
  });
  ...
  function renderVarBody(body, key) {
    ...
    if (def.numeric) {
      ...
      cb.addEventListener("change", () => {
        ...
        if (v.mode === "map") autoSuggestValueMap(key);
        else syncNumericCategories(key); // reconcile in case map mode renamed a bracket
        resetTargetsToSample(key);
        renderVarBody(body, key);
        renderDiagnostics();
      });
  ...
  // Numeric-mode editor: one row per bracket, each with a single editable
  // "starts at" value and a read-only computed label right next to it.
  // There's exactly one thing to type per bracket now. The label can never
  // disagree with the boundary because it's never typed independently.
  function buildBinEditor(key, body) {
    ...
    p.textContent =
      "Looks numeric. Set where each bracket starts, and the label to the right updates on its own. Removing a bracket merges its range into the one above it.";
    wrap.appendChild(p);
    ...
    v.categories.forEach((cat, i) => {
      ...
      row.className = "bin-row-item";

      const startField = document.createElement("div");
      startField.className = "bin-start-field";
      const startLabel = document.createElement("span");
      startLabel.className = "bin-start-caption";
      startLabel.textContent = i === 0 ? "Starts at" : "Then at";
      ...
      cutoffInput.setAttribute("aria-label", "Value where bracket " + (i + 1) + " starts");
      cutoffInput.addEventListener("change", () => {
        ...
        v.cutoffs[i] = val;
        v.cutoffs.sort((a, b) => a - b); // keep boundaries in order regardless of edit order
        syncNumericCategories(key);
        renderVarBody(body, key);
        renderDiagnostics();
      });
      startField.appendChild(startLabel);
      startField.appendChild(cutoffInput);
      row.appendChild(startField);

      const arrow = document.createElement("span");
      arrow.className = "bin-arrow-item";
      arrow.textContent = "\u2192";
      row.appendChild(arrow);

      const computedLabel = document.createElement("span");
      computedLabel.className = "bin-computed-label";
      computedLabel.textContent = cat;
      computedLabel.title = "Computed automatically from the boundary values";
      row.appendChild(computedLabel);

      const removeBtn = document.createElement("button");
      removeBtn.type = "button";
      removeBtn.className = "cat-remove-btn";
      removeBtn.textContent = "×";
      removeBtn.title = i === 0 ? "Remove this bracket (excludes ages below the next boundary)" : "Remove this bracket (merges it into the one above)";
    ...
    addBtn.addEventListener("click", () => {
      ...
      renderVarBody(body, key);
      renderDiagnostics();
      const inputs = body.querySelectorAll(".bin-input");
    ...
    v.categories.forEach((cat) => {
      ...
      input.addEventListener("change", () => {
        ...
        if (newName !== current && v.categories.indexOf(newName) !== -1) {
          input.value = current; // duplicate name, revert
          return;
        }
  ...
  function renderDiagnostics() {
    stratEnforceRow.hidden = !state.vars[STRATIFICATION_KEY].enabled;
    if (!state.data) {
    ...
    const baseWeights = computeBaseWeights();
    if (Object.keys(active).length === 0 && !baseWeights) {
      diagnosticsEl.innerHTML = '<p class="empty-diag">Turn on at least one variable, or the stratification design, above to compute weights.</p>';
    ...
    // Show the stratification margin whenever base weights are in play, even
    // if it isn't an ongoing IPF margin (seed-only mode), so it's visible
    // how the stratum balance actually landed either way, not just when it's
    // being actively re-enforced.
    const marginKeys = Object.keys(active);
    if (baseWeights && marginKeys.indexOf(STRATIFICATION_KEY) === -1) {
      marginKeys.unshift(STRATIFICATION_KEY);
    }
...
// ==================================================================
// "How this works" explainer modal. Shown automatically once per
// browser (via localStorage), always reachable afterward through the
// footer link. Kept as its own closure, independent of the main app
// above, since it has no dependency on uploaded data or app state.
// ==================================================================
(function () {
  const EXPLAINER_SEEN_KEY = "raking-app-explainer-seen";
  const backdrop = document.getElementById("explainer-backdrop");
  const openBtn = document.getElementById("explainer-open");
  const closeBtn = document.getElementById("explainer-close");
  const gotItBtn = document.getElementById("explainer-got-it");
  if (!backdrop || !openBtn || !closeBtn || !gotItBtn) return;
  ...
  function closeExplainer() {
    ...
    try {
      ...
      // localStorage unavailable (private browsing, etc.), harmless, it
      // just means the explainer auto-shows again next visit instead of
      // remembering that it was already dismissed
    }
    if (lastFocused && typeof lastFocused.focus === "function") lastFocused.focus();
  }

  openBtn.addEventListener("click", openExplainer);
  closeBtn.addEventListener("click", closeExplainer);
  gotItBtn.addEventListener("click", closeExplainer);
  backdrop.addEventListener("click", (e) => {
    if (e.target === backdrop) closeExplainer();
  });

  let alreadySeen = false;
  try {
    alreadySeen = localStorage.getItem(EXPLAINER_SEEN_KEY) === "1";
  } catch (e) {
    alreadySeen = false;
  }
  if (!alreadySeen) openExplainer();
})();
