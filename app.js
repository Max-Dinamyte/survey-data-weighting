(function () {
  ...
  function rawColumnStats(col, weights) {
    const counts = {};
    const wsums = {};
    let total = 0;
    let wtotal = 0;
    ...
    const rows = Object.keys(counts).map((label) => ({
      label,
      n: counts[label],
    ...
    return { rows, distinctCount: rows.length, total };
  }
  ...
  function renderLookupTable() {
    ...
    const stats = rawColumnStats(col, state.lastWeights);
    ...
    stats.rows.slice(0, LOOKUP_MAX_ROWS).forEach((r) => {
      const tr = document.createElement("tr");
      [r.label, r.n.toLocaleString(), round(r.unweightedPct * 100, 1) + "%", round(r.weightedPct * 100, 1) + "%"].forEach(
        (c) => {
  ...
  function crosstabRowKey(rowCols, row) {
    return rowCols
      .map((c) => {
        const raw = row[c];
        const s = raw == null ? "" : String(raw).trim();
  ...
  function crosstabStats(rowCols, colCol, weights) {
    ...
    for (let i = 0; i < state.data.length; i++) {
      ...
      const rowKey = crosstabRowKey(rowCols, row);
      const cv = row[colCol];
    ...
    return { rowCounts, rowTotals, colTotals, grandTotal };
  }
  ...
  function renderCrosstab() {
    ...
    let rowKeys = Object.keys(stats.rowTotals).sort((a, b) => stats.rowTotals[b] - stats.rowTotals[a]);
    let colKeys = Object.keys(stats.colTotals).sort((a, b) => stats.colTotals[b] - stats.colTotals[a]);
